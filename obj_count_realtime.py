...
#false negatives 
def custom_counting(input_video, detection_graph, category_index, is_color_recognition_enabled,targeted_objects, 
                    fps, 
                    width, 
                    height,
                    roi=0,
        ...
        #enable following two lines if you would like video output
        ...
        #change code below based on roi
        #height = 0
        #width = 0
    
        #cap = cv2.VideoCapture("rtsp://172.17.62.28")
       ...
       # frame_numb = 0
        ...
        with detection_graph.as_default():
          with tf.Session(graph=detection_graph) as sess:
            ...
            with open('./fullstreet.csv', mode='w+') as street_file:
                ...
                #print(cap.isOpened())
                while(1):
                    client = rtsp.Client('rtsp://172.17.62.43')
                    #client = rtsp.Client('rtsp://172.17.62.22')
                    frame= client.read()
                    frame.show()
                    client.close()	
                    #modifcation for live video
                    ...
                    #establish height and width of frame for text display
                    ...
                    if True:
                            ...
                            numb_motorcycles = 0 #number of motorcycles detected 
                            ...
                            counter, csv_line, counting_mode = vis_util.visualize_boxes_and_labels_on_image_array_x_axis(framecount,
                                                                                                                         input_frame,
                                                                                                                         1,
                            ...
                            #cv2.line(input_frame, (0, roi), (width,roi), (255,0,0), 20)
                            ...
                            print(counting_mode)
                            #if bicycle is not in dict display nothing 
                            ...
                            bog_dt = loc_dt.astimezone(bogota)
                            fmt = "%Y-%m-%d %H:%M:%S"
                            timeform = bog_dt.strftime(fmt)
                            print(timeform)
                            #overall report
                            #print(str(numb_bicycles)+"bicycle, "+str(numb_motorcycles)+"motorcycle, "+str(numb_taxis)+"taxis, "+
                             #   +str(numb_redbus)+"redbus")
                            
                            #write time and number of objects to csv
                            street_writer.writerow([timeform,str(numb_bicycles),str(numb_motorcycles),str(numb_redbus),str(numb_taxis)])
        ...
        with detection_graph.as_default():
          with tf.Session(graph=detection_graph) as sess:
            ...
            while(cap.isOpened()):
                ...
                # when the vehicle passed over line and counted, make the color of ROI line green
                cv2.line(input_frame, (roi, 0), (300, height), (0, 0xFF, 0), 20)
                if counter == 1:
